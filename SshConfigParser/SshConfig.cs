...
public class SshConfig
{
  ...
  public void Add(SshHost host) => Add(host.Properties);
  ...
  public void Add(IDictionary opts)
  {
    ...
    foreach (var line in nodes)
      if (RE_SECTION_DIRECTIVE.IsMatch(line.Param))
        foreach (var subline in line.Config.nodes)
    ...
    config.nodes[config.nodes.Count - 1].After += Environment.NewLine;
  }
  ...
  /// <param name="host"></param>
  /// <returns></returns>
  public ConfigNode FindNodeByHost(string host) => FindNode("Host", host);
  ...
  /// <param name="match"></param>
  /// <returns></returns>
  public ConfigNode FindNodeByMatch(string match) => FindNode("Match", match);
  ...
  private ConfigNode? FindNode(string findBy, string find)
  {
    ...
    var query = from line in this.nodes
  ...
  public override string ToString()
  {
    ...
    void Format(ConfigNode line)
    {
      ...
      line.Config?.nodes.ForEach(Format);
    }

    nodes.ForEach(Format);
  ...
  /// <returns></returns>
  internal static SshConfig Parse(string str)
  {
    var i = 0;
    var chr = Next();
  ...
  public void Add(ConfigNode node) => nodes.Add(node);

  public int Count => nodes.Count;

  public ConfigNode this[int index] => nodes[index];

  public IEnumerable<ConfigNode> AsEnumerable() => nodes;
}
